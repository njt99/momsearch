			...
			if (numFound == 1) { // exactly one lattice point possible
				...
				if (isImpossible) {
					...
					for (int i = 0; i < -yLattice; ++i)
						relator += 'N';
					if (momFinder) {
						if (momFinder->find(params, relator))
							return 7;
					}
					return 6;
				}
...
bool TestCollection::evaluateCenter(int index, Box& box)
{
	...
	switch(index) {
		...
		case 6: {
			...
			return g_latticeArea >= maximumArea;
		}
		default:
...
void TestCollection::setMomFinder(FindMomManifold* finder)
{
	momFinder = finder;
}
