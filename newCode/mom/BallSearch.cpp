...
namespace BallSearchImpl {
	...
	struct LargerBallSize : public binary_function<const WordPair&, const WordPair&, bool>
	{
...
//			printf("pushWordPair(%s,%s)\n", pair.firstWord->name.c_str(), pair.secondWord->name.c_str());
	...
	{
		pairs.sort(LargerBallSize());
		for (;;) {
			WordPair& largest(pairs.front());
